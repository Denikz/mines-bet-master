...
function aposta() {
     ...
     if (apostando == 0) {
        ...
        if (apost == '' || apost == 0) {
            ...
            alert("Ваш баланс составляет ₸ " + saldo + " - Пополните депозит.")
        }
        else if (apost > saldo.toFixed(2)) {
            alert("Ваш баланс составляет ₸ " + saldo + ' .Введите совместимое значение.')
        } else {
            saldo = saldo - apost;
            document.getElementById('valor').innerHTML = '₸ ' + saldo
            gerarBomb()
            document.getElementById('inp').value = 0;
            document.getElementById('alerta').innerText = 'Начатый раунд-сумма ставки: ₸ '+apost
            blurs(1);
            apostando = 1;
...
function para () {
    if (apostando == 1) {
        if (ganho <= 0) {
            ...
            document.getElementById('alerta').innerText = 'Вы выиграли - ₸ '+ganho.toFixed(2)  
            document.getElementById('valor').innerHTML = '₸ ' + saldo.toFixed(2)
            document.getElementById('btApost').innerText = 'Сделать ставку'
...
function mudarBotao(valor, resp) {
    if (resp == 0) {
        ...
        .then(dados => {
            ...
            document.getElementById('alerta').innerText = 'Вы проиграли - ₸ '+apost 
            document.getElementById('btApost').innerText = 'Сделать ставку'
...
function reset() {
    document.getElementById('alerta').innerText = 'Ожидание ставки...'
    document.getElementById('alerta').style.color = 'white'
    document.getElementById('btApost').innerText = 'Сделать'
